...
export function ForecastedTrendsCard({ items }: Props) {
  return (
    <div className="rounded-xl border border-primary/40 bg-card p-5 shadow-sm relative">
      <div className="text-[11px] font-semibold tracking-wider text-muted-foreground mb-4">
        ...
        {items.map((item) => (
          <div key={item.label}>
            <div className="flex justify-between text-xs mb-1.5">
              ...
              <span className={`font-semibold ${item.statusColor === "rose" ? "text-rose-400" : item.statusColor === "amber" ? "text-amber-400" : "text-primary"}`}>
                {item.value}
              </span>
            </div>

  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card p-6 shadow-sm h-full flex flex-col justify-between">
      <div className="flex items-center justify-between mb-5">
        ...
        <MoreHorizontal className="h-4 w-4 text-foreground cursor-pointer" />
      </div>
      ...
      <div className="space-y-4">
        {insights.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="rounded-lg border border-border/50 bg-background/50 p-4 shadow-sm flex items-start gap-3"
            >
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-medium text-foreground">{item.timestamp}</span>
                </div>
                <p className="text-xs text-foreground mt-1 leading-relaxed">{item.description}</p>
              </div>
            </div>
          );
        })}
